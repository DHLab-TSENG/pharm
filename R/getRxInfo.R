get.rxinfo <- function(df, RxCuiColName = RxCui, cores = 8){
  ...
  RxNormInFoData = foreach(i = 1:nrow(df),
                         ...
                         .packages = "jsonlite") %dopar% {
                           rxinfo <- fromJSON(paste0("https://rxnav.nlm.nih.gov/REST/RxTerms/rxcui/", df$RxCui[i], "/allinfo"))
  ...
  RxCui_InFo <- left_join(df,RxNormInFoData, by = "RxCui")
  colnames(RxCui_InFo)[colnames(RxCui_InFo)=="RxCui"] <- deparse(substitute(RxCuiColName))
